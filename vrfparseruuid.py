...
def parse_vrf_hwid(vrfname):
    ...
    r_username = "admin"
    r_password = "Pensando!2345"
    ssh = netmiko.ConnectHandler(**{'device_type': 'aruba_os', 'ip': router_ip, 'username': r_username, 'password': r_password})
    ssh.send_command("start-shell", expect_string="$")
    vrfs=ssh.send_command("curl localhost:9007/api/vrfs/ | jq .", expect_string="$ ")
    print(vrfs)
    exit()
    ...
    vrf_out = ssh.send_command("pdsctl show vpc --status -i " + vrf_uuid[0], expect_string="$ ")
    vrf=[]
...
vrf_flows = ssh.send_command("pdsctl show flow --vpcid " + vrf_hwid)
print(vrf_flows)